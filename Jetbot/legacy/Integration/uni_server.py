...
class Midas:
    ...
    def predict(self, img):
        ...
        if PLOT:
            ...
            np.save(f"./img/np/np_{IMG_ITERATOR}", prediction.cpu().numpy())
...
class MidasInterpreter:
    ...
    def find_obstacles(self,depth_image):
        ...
        left_free = (left_depth < self.MIN_SAFE_DISTANCE or left_columns_count < self.BOK_GROUP)  and left_count < self.GROUP_COUNT and left_average < self.MIN_SAFE_DISTANCE_MEAN
        mid_free = mid_depth < self.MIN_SAFE_DISTANCE and mid_count < self.GROUP_COUNT and mid_average < self.MIN_SAFE_DISTANCE_MEAN
        right_free = (right_depth < self.MIN_SAFE_DISTANCE or right_columns_count < self.BOK_GROUP) and right_count < self.GROUP_COUNT and right_average < self.MIN_SAFE_DISTANCE_MEAN
