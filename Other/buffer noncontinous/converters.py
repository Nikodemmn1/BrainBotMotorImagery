...
class EEGDataConverter:
            ...
            for i in range(markers.size):
                if markers[i] != curr_marker:
                    if curr_marker in range(1, 7):
                        ...
                        buffer_filled = 0
                        for snippet in np.split(recording[:, :recording_len_divisible],
                            ...
                            if buffer_filled + self.OVERLAP >= self.BUFFER_LENGTH:
                                ...
                                self.labels.append(curr_marker)
                            else:
    ...
    def _calculate_psd_fft(self):
        self.converted_data = [np.apply_along_axis(self._calculate_psd_fft_channel, 1, recording) for recording in
                               self.converted_data]
